...
function valid(){
	//alert("fuck");
	if(!$("#highaddempFrm").validate().form()) return;
	$.ajax({
		url:"highaddemp.do",
		data:{comid:$("#comid").val(), empid:$("#empid").val(), empkey:$("#empkey").val(),empname:$("#empname").val()},
		success:function(data) {
			//alert(data);
			if(data == '1') {
				alert("添加成功！");
				window.location.href = "choosecom.do";
			}
			else if(data == '0') {
				alert("已有该人员ID，ID重复！");
			} 
